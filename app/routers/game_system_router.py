...
def post_add_game_system(
        ...
        game_system_service: GameSystemService = Depends(get_protected_game_system_service)
):
...
def get_all_game_systems(
        game_system_service: GameSystemService = Depends(get_public_game_system_service)
):
